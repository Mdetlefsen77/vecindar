...
export default async function ReportesPage({
  ...
  const generadoPor = `${session.user.name ?? "—"} (${ROL_LABEL[session.user.role] ?? session.user.role})`;
      ...
      <tbody role="presentation">
        <tr role="presentation">
          ...
          <div className="hidden print:flex print:flex-col print:items-center print:justify-center print:min-h-[22cm] print:break-after-page print:text-center gap-3">
            <Image src="/images/icon-192.png" alt="" width={80} height={80} />
            ...
            <div className="mt-10 space-y-1.5 text-sm text-gray-700">
              <p className="text-xl font-semibold">{modelo.periodo.etiqueta}</p>
              <p>
                Emitido el {new Date(modelo.generadoAt).toLocaleString("es-AR")}
              </p>
              ...
              {modelo.incidentes.antiguedadBacklog && (
                <Campo label="Antigüedad del backlog">
                  <span className="text-sm font-semibold">
                    ...
                    <span className="font-normal text-gray-400">
                      {" "}
                      (máx. {modelo.incidentes.antiguedadBacklog.maximoDias}d)
                    </span>
                  </span>
                </Campo>
              )}
          ...
          <Seccion titulo="Alertas de pánico (SOS)">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              ...
              <Campo label="% atendidas bajo umbral">
                <ValorConUmbral v={modelo.panico.pctBajoUmbral} unidad="%" />
              </Campo>
            </div>
          </Seccion>
        ...
        <tr role="presentation">
          <td role="presentation">
            <div className="hidden print:flex justify-between text-[10px] text-gray-500 border-t border-gray-300 pt-1 mt-3">
              ...
              <span>{clasificacion} · Generado automáticamente por Vecindar</span>
            </div>
          </td>
        </tr>
      </tfoot>
