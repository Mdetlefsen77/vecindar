...
Tailwind v4 changes the configuration approach with `@theme` inline in CSS:
...
**`app/globals.css`** (use @theme inline):
...
:root {
  ...
  --color-app-warning: #f97316;
}

@theme inline {
  --color-app-danger: var(--color-app-danger);
  --color-app-warning: var(--color-app-warning);
}
```

**Benefits**:

- ✅ No `tailwind.config.js` needed (use `tailwind.config.ts` for types)
- ✅ CSS variables integrate natively
- ✅ `@theme` inline defines custom utilities and colors
...
## Custom Colors & Semantic Design (v4 Best Practice)

Define application-specific colors in `tailwind.config.ts` to maintain consistency:
...
const config: Config = {
  theme: {
    extend: {
      colors: {
        app: {
          danger: "#EF4444", // Critical incidents, errors
          warning: "#F97316", // Warnings, alerts
          success: "#22C55E", // Resolved incidents, confirmations
          info: "#3B82F6", // Information, notifications
          gold: "#EAB308", // Special, highlights
        },
      },
    },
  },
...
**Usage - Consistent alert badges**:

```html
<!-- Before: Hardcoded colors scattered -->
<span class="bg-red-100 text-red-700">Danger</span>
<span class="bg-orange-100 text-orange-700">Warning</span>

<!-- After: Uses app colors -->
<span class="bg-rose-50 text-app-danger font-semibold">Danger</span>
<span class="bg-amber-50 text-app-warning font-semibold">Warning</span>
```

## Reusable Component Pattern

Extract Tailwind classes into extracted constants to avoid repetition and improve maintainability:
...
const badgeStyles: Record<BadgeVariant, string> = {
  danger: 'bg-rose-50 text-app-danger px-3 py-1 rounded-md font-semibold',
  warning: 'bg-amber-50 text-app-warning px-3 py-1 rounded-md font-semibold',
  success: 'bg-emerald-50 text-app-success px-3 py-1 rounded-md font-semibold',
};
...
// Usage everywhere in your app - consistent, maintainable
...
- ✅ Easier to update colors across the app
- ✅ Type-safe variant selection
- ✅ Cleaner JSX without long classNames
